...
private final class MacSynthKitEngine {
  private var audioEngine: AVAudioEngine?
  private var synths: [String: MacSynthHandle] = [:]
  ...
  func createSynth(spec: MacSynthSpec) throws -> String {
    try ensureInitialized()
    let synthId = "mac_synth_\(nextSynthId)"
    nextSynthId += 1
    synths[synthId] = try makeSynthHandle(id: synthId, spec: spec)
    return synthId
  }

  func updateSynth(id: String, spec: MacSynthSpec) throws {
    try ensureInitialized()
    guard let handle = synths[id] else {
      throw MacSynthKitError.unknownSynth(id)
    }
  ...
  func triggerNote(
    ...
    guard let handle = synths[synthId] else {
      throw MacSynthKitError.unknownSynth(synthId)
    }
  ...
  private func ensureInitialized() throws {
    if audioEngine == nil {
      throw MacSynthKitError.invalidArguments("Call initialize() before using the plugin.")
    }
  }

  private func makeSynthHandle(id: String, spec: MacSynthSpec) throws -> MacSynthHandle {
    guard let engine = audioEngine else {
      throw MacSynthKitError.invalidArguments("Audio engine is not ready.")
    }
    ...
    guard let format = AVAudioFormat(
      ...
      throw MacSynthKitError.engineFailure("Unable to configure the macOS audio format.")
    }

    let state = MacSourceSynthState(sampleRate: sampleRate, spec: spec)
    ...
    do {
      ...
      throw MacSynthKitError.engineFailure(
        "Failed to start the macOS audio engine: \(error.localizedDescription)"
      )
    }

    return MacSynthHandle(id: id, state: state, sourceNode: sourceNode)
  }

  private func play(handle: MacSynthHandle, frequencyHz: Double, durationMs: Int, velocity: Double) {
  ...
  private func schedule(on handle: MacSynthHandle, afterMs delayMs: Int, block: @escaping () -> Void) {
    let workId = UUID()
  ...
  private func cancel(handle: MacSynthHandle) {
  ...
  private func detach(handle: MacSynthHandle) {
    guard let engine = audioEngine else {
      return
    }
...
private final class MacSynthHandle {
  init(
    id: String,
    state: MacSourceSynthState,
  ...
  let id: String
  let state: MacSourceSynthState
  let sourceNode: AVAudioSourceNode
  var scheduled: [UUID: DispatchWorkItem] = [:]
}

private final class MacSourceSynthState {
  init(sampleRate: Double, spec: MacSynthSpec) {
...
private struct MacSynthSpec {
  init(args: [String: Any]) throws {
    waveform = args["waveform"] as? String ?? "sine"
    volume = MacSynthKitNumeric.double(args["volume"], fallback: 0.8)
    envelope = try MacEnvelopeSpec(args: args["envelope"] as? [String: Any] ?? [:])
    filter = try MacFilterSpec(args: args["filter"] as? [String: Any] ?? [:])
  }

  let waveform: String
  let volume: Double
  let envelope: MacEnvelopeSpec
  let filter: MacFilterSpec
}

private struct MacEnvelopeSpec {
  init(args: [String: Any]) throws {
    attackSeconds = Double(MacSynthKitNumeric.int(args["attackMs"], fallback: 10)) / 1000
    decaySeconds = Double(MacSynthKitNumeric.int(args["decayMs"], fallback: 120)) / 1000
    sustain = MacSynthKitNumeric.double(args["sustain"], fallback: 0.75)
    releaseSeconds = Double(MacSynthKitNumeric.int(args["releaseMs"], fallback: 240)) / 1000
  }
...
private struct MacFilterSpec {
  init(args: [String: Any]) throws {
    enabled = args["enabled"] as? Bool ?? false
    cutoffHz = MacSynthKitNumeric.double(args["cutoffHz"], fallback: 1800)
  }
...
private enum MacSynthKitError: LocalizedError {
...
private enum MacSynthKitNumeric {
